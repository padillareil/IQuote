...
function loadHome() {
    $.post("dirs/home/components/home.php", {
        ...
        var BranchType = $("#branch-type").val();
        
        if (Bcode === 'HO' || Bcode === 'ICBU') {
            loadAllBranch();
        } 
        else if (BranchType === 'ONLN'|| BranchType === 'ICBU' || BranchType === 'HEAD OFFICE') {
            loadAllBranch();
        } else {
            loadMyBranch();
        }
...
/*function loadForm4() {
    ...
    // check only inside display-orders
    $("#display-orders .order-form").each(function () {
        let $row = $(this);
        let $quantity = $row.find(".quantity");

        if ($.trim($quantity.val()) === "") {
            isValid = false;
            $quantity.addClass("is-invalid"); // highlight the missing field
        } else {
    ...
    if (!isValid) {
        Swal.fire({
            ...
            text: 'Please enter quantities before proceeding.',
...
function loadMyBranch() {
    $.post("dirs/home/actions/get_mybranch.php", {}, function(data) {
        var response = JSON.parse(data);
        if ($.trim(response.isSuccess) === "success") {
            var branches = response.Data;
            var $branch = $("#branch");
