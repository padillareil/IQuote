...
function load_CNCManager() {
    $.post("dirs/acc-approver/components/main.php", {
    }, function (data){
        $("#load_CNCBranch").html(data);
    });
}
...
function createHeadOfficeStaff() {
    ...
    // Submit via AJAX
    $.post("dirs/acc-approver/actions/save_staff.php", {
        Fullname: Fullname,
        Position: Position,
        Username: Username,
        Password: Password,
...
function mdl_UserAccount(UserName){
    ...
    $.post("dirs/acc-approver/actions/get_userperformance.php",{
        UserName : UserName
    },function(data){
        response = JSON.parse(data);
        if(jQuery.trim(response.isSuccess) == "success"){
...
function commitEnableHO(){
    ...
    var Status      = 'ENABLE';
    $.post("dirs/acc-approver/actions/update_enable.php", {
        UserName : UserName,
        Status     : Status,
    }, function(data){
...
function disableuserHO(){
    var UserName            = $("#get-username").val();
    $.post("dirs/acc-approver/actions/update_disable.php", {
        UserName    : UserName,
    }, function(data){
        if($.trim(data) == "success"){
            $("#mdl-update-accountho").modal("hide");
...
function goRemove() {
    ...
    $.post("dirs/acc-approver/actions/update_removed.php", {
        UserName: UserName,
        Status: Status,
    }, function(data) {
        if ($.trim(data) == "success") {
...
function getUsername(UserName) {
    $.post("dirs/acc-approver/actions/get_username.php", {
        UserName: UserName
    }, function(data) {
