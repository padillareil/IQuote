...
$User = $conn->prepare("
    ...
    FROM USR
    WHERE Position = 'Coordinator'    ORDER BY Name ASC
");
$User->execute();
$rows = $User->fetchAll(PDO::FETCH_ASSOC);
?>
<div class="d-flex justify-content-between align-items-center mb-3">
    <button class="btn btn-outline-secondary" type="button" onclick="mdlCoordinator()">Create Coodinator</button>

    <div class="d-flex align-items-center gap-2">
        <input type="search" name="search-coordinator" id="search-coordinator" class="form-control" placeholder="Search" style="max-width: 250px;">
    </div>
</div>
...
<script>
  $(document).ready(function(){
        $("#search-coordinator").on("keyup", function(){
