<!-- Create Unit Terms and Condition -->
<div class="d-flex justify-content-between align-items-center mb-3 flex-wrap">
  <div>
    <h4 class="text-muted mb-0">Terms and Conditions</h4>
<!--     <small class="text-muted">Set the agreement details and policies for this unit.</small> -->
  </div>
</div>
<hr>
<div class="mt-2 mb-2">
	<div id="termscondition-container"></div>
</div>
<hr>
<div class="justify-content-end d-flex mt-3 mb-3">
		<button class="btn btn-secondary me-2" type="button" onclick="EditreturnForm5()">Back</button>
	<button class="btn btn-danger" type="button" onclick="EditloadForm5()">Next</button>
</div>