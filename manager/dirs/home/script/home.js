...
function loadForm2() {
    ...
    if (customerType === "Corporate Government") {
        if (!$("#companyname").val().trim()) {
            ...
            console.log("Missing company name for Corporate Government");
        } else {
        ...
        if (!$("#companyname").val().trim()) {
            isValid = false;
            $("#companyname").addClass("is-invalid");
            console.log("Missing company name for Corporate Private");
        } else {
            $("#companyname").removeClass("is-invalid");
        }
    }
        ...
        if (!$("#companyname").val().trim()) {
            isValid = false;
            $("#companyname").addClass("is-invalid");
            console.log("Missing company name for Private");
        } else {
            $("#companyname").removeClass("is-invalid");
        }
    }
    ...
    if (!isValid) {
        alert("Please check for missing fields.");
        return;
    }

    // ✅ Step 5: Continue to next tab
...
/*Load Branch for assigned branch*/
...
/*Load Branch for managers branch*/
function loadAllBranchManager() {
    $.post("dirs/home/actions/get_brancmanageruser.php", {}, function(data) {
        var response = JSON.parse(data);
        if ($.trim(response.isSuccess) === "success") {
            var branches = response.Data;
            var $branch = $("#branch");
...
/*Update load corpcode based on RM Business Review*/
function loadCorpCode(BranchName) {
    $.post("dirs/home/actions/get_corpcode.php", { Branch: BranchName }, function(data) {
