  ...
  $("#mdl-review-quotation").modal('show');
    $.post("dirs/quotation_list/actions/get_reviewquotation.php",{
        ...
        if(jQuery.trim(response.isSuccess) == "success"){
            ...
            $("#remarks").text(formatComma(response.Header.Remarks));


           const orders = response.Orders;
           var orderitem = "";
            orders.forEach((itm, index) => {
                orderitem += `
                    <tr class="border-bottom">
                        <td class="text-center">${index + 1}</td>
                        <td class="ps-3 py-3">
                            <div class="text-dark">
                                ${itm.ItemBrand} / ${itm.ItemModel}
                            </div>
                        </td>
            ...
            $("#table-order-items").html(orderitem);


            const termscondition = response.Terms;
            var trmscon = "";
             termscondition.forEach((trms, index) => {
                 trmscon += `
                     <li class="small text-muted mb-2 d-flex align-items-start">
                         <span class="me-2 text-muted">•</span>
                         <h6>
                            ${trms.TermsAndCondition}
                         </h6>
                     </li>
                 `;
             });

             $("#termsand_conditions").html(trmscon);

             const warranty = response.Warranty;
             var wrrnty = "";
              warranty.forEach((wrrny, index) => {
                  wrrnty += `
                      <li class="small text-muted mb-2 d-flex align-items-start">
                          <span class="me-2 text-muted">•</span>
                          <h6>
              ...
              $("#warranty_conditions").html(wrrnty);



        }else{
            alert(jQuery.trim(response.Orders));
        }
    });
}